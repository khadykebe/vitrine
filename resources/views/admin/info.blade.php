        ...
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              ...
              <button type="button" class="btn btn-primary mb-3" data-toggle="modal" data-target="#addStudentModal"><i class="fa fa-add" aria-hidden="true"></i></button>
              ...
              <table class="table datatable">
                ...
                <tbody>
                    @foreach ($descrips as $descrip)
                        <tr>
                            <td>{{ $descrip->nom }}</td>
                            <td>{{ $descrip->adresse }}</td>
                            <td>{{ $descrip->email }}</td>
                            <td>{{ $descrip->Historique }}</td>
                            <td>{{ $descrip->mission }}</td>
                            <td>{{ $descrip->valeur }}</td>
                            <td>
                                <form action="{{ route('deleteDescrip', $descrip->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                ...
                <table class="table datatable">
                  ...
                  <tbody>
                      @foreach ($ecoles as $ecole)
                          <tr>
                              <td>{{ $ecole->NbreClasse }}</td>
                              <td>{{ $ecole->NbreEleve }}</td>
                              <td>{{ $ecole->NbreProf }}</td>
                              <td>{{ $ecole->historique }}</td>
                              <td>{{ $ecole->presentation }}</td>
                              <td>{{ $ecole->MotDirecteur }}</td>
                              <td>
                                  <form action="{{ route('deleteDescrip', $ecole->id) }}" method="POST">
                                      @csrf
                                      @method('DELETE')
                                      <button type="submit" class="btn btn-danger">Delete</button>
                                  </form>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addStudentModalLabel">Description</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('descrip') }}" method="POST">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
                        <label for="first_name">Nom Ecole</label>
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addStudentModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addStudentModalLabel">Description</h5>
            ...
            <form action="{{ route('addEcole') }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label for="first_name">Nbre Classe</label>
                        <input type="text" name="NbreClasse" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label for="address">Nbre Eléves</label>
                        <input type="text" name="NbreEleve" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label for="email">Nbre Enseignants</label>
                        <input type="text" name="NbreProf" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label for="Historique">Historique</label>
                        <textarea  type="text" name="historique" class="form-control"></textarea>
                    ...
                    <div class="form-group">
                        <label for="mission">presentation</label>
                        <input type="text" name="presentation" class="form-control">
                    </div>
                    <div class="form-group">
                        <label for="value">MotDirecteur</label>
                        <input type="text"  name="MotDirecteur" class="form-control">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">fermer</button>
